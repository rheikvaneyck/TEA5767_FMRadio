...
#ifndef TEA5767_FMRadio.h
#define TEA5767_FMRadio.h
...
enum 
{
  ...
  // Info Byte #1
  RF        = 0x80, // Ready flag is 1 if station is found
  BLF       = 0x40, // Band limit flag is 1 if band limit is reached
  // Info Byte #3
  STEREO    = 0x80 // Stereo indication is 1 if stereo reception
};
...
public:
  ...
  uint8_t get_info_bit(uint8_t data_byte, uint8_t config_bit);
  uint8_t get_ready_flag(void);
  uint8_t get_band_limit_flag(void);
  uint32_t get_frequency(void);
  float get_frequency_MHz(void);
  uint8_t get_stereo_reception(void);
  ...
  float get_config_frequency_MHz(void);
